...
impl OrganizationRepository {
    ...
    pub async fn update(&self, id: Uuid, update: crate::api::dto::UpdateOrganizationRequest) -> AppResult<organization::Model> {
        ...
        if let Some(name) = update.name { active.name = Set(name); }
        if let Some(organization_type) = update.organization_type { active.organization_type = Set(organization_type); }
        if let Some(registration_number) = update.registration_number { active.registration_number = Set(Some(registration_number)); }
        if let Some(sector) = update.sector { active.sector = Set(Some(sector)); }
        if let Some(region) = update.region { active.region = Set(Some(region)); }
        if let Some(contact_email) = update.contact_email { active.contact_email = Set(Some(contact_email)); }
        if let Some(contact_phone) = update.contact_phone { active.contact_phone = Set(Some(contact_phone)); }
        if let Some(address) = update.address { active.address = Set(Some(address)); }
        if let Some(federation_id) = update.federation_id { active.federation_id = Set(Some(federation_id)); }
