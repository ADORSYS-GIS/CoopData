...
fn protected_routes() -> Router<AppState> {
    Router::new()
        .route("/organizations", axum::routing::get(handlers::list_organizations).post(handlers::create_organization))
        .route("/organizations/{id}",
            axum::routing::get(handlers::get_organization)
                .patch(handlers::update_organization)
                .delete(handlers::delete_organization)
        )
        .route("/users", axum::routing::get(handlers::list_users).post(handlers::create_user))
        .route("/users/{id}",
            axum::routing::get(handlers::get_user)
                .patch(handlers::update_user)
                .delete(handlers::delete_user)
        )
        .route("/users/{id}/assign-role", axum::routing::post(handlers::assign_role))
}
