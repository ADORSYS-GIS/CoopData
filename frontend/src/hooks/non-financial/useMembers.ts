...
async function fetchMembers(params?: NfListParams): Promise<PaginatedResponse<MemberResponse>> {
  ...
  return json as PaginatedResponse<MemberResponse>;
}

async function fetchMember(id: string): Promise<MemberResponse> {
  const token = await getAccessToken();
...
async function createMember(body: CreateMemberRequest): Promise<MemberResponse> {
  const token = await getAccessToken();
