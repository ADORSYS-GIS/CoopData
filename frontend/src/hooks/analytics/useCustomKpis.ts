...
export function useCustomKpis() {
  ...
  const kpisQuery = useOfflineQuery({
    queryKey: ["custom-kpis", i18n.language?.split("-")[0] || "en"],
    cacheTable: "analytics",
    cacheKey: `custom-kpis-${i18n.language?.split("-")[0] || "en"}`,
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (payload: {
      ...
      return runMutation<unknown>("/api/v1/ministry/custom-kpis", "POST", {
        body: payload,
        ...
        online: async () => {
          ...
          if (error)
            throw new Error(
              (error as { message?: string })?.message || "Failed to create Custom KPI",
            );
          return data;
        },
      });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return runMutation<void>("/api/v1/ministry/custom-kpis/{id}", "DELETE", {
        pathParams: { id },
        online: async () => {
  ...
  const evaluateMutation = useMutation({
    mutationFn: async (formula: string) => {
      return runMutation<{
        ...
        is_valid: boolean;
        error?: string | null;
      }>("/api/v1/ministry/custom-kpis/evaluate", "POST", {
        body: { formula },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      return runMutation<unknown>("/api/v1/ministry/custom-kpis/{id}", "PUT", {
        pathParams: { id },
        body: payload,
        optimisticData: payload,
        online: async () => {
          ...
          if (error)
            throw new Error(
              (error as { message?: string })?.message || "Failed to update Custom KPI",
            );
          return data;
        },
      });
