...
/**
 * openapi-fetch's `GET` is generic over every route, so a single concrete
 * stub type cannot exist. Test stubs only ever carry `{ data }` payloads,
 * so they are funneled through this helper once, at the type boundary.
 */
function stubGetResponse(body: unknown): never {
...
describe("offlineSeeder", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(apiClient.GET).mockResolvedValue(stubGetResponse({ data: [] }));
  ...
  describe("Error Resilience (safeFetch)", () => {
    it("should continue executing subsequent endpoints if one fails", async () => {
      Object.defineProperty(navigator, "onLine", { value: true, writable: true });
      vi.mocked(getUserProfile).mockReturnValue(profileFixture({ id: "123" }));
      ...
      vi.mocked(apiClient.GET).mockResolvedValue(stubGetResponse({ data: [{ dummy: 1 }] }));
  ...
  describe("Role-Based Execution Branches", () => {
    ...
    it("should fetch and cache ministry-specific endpoints", async () => {
      vi.mocked(getUserProfile).mockReturnValue(profileFixture({ id: "min-1", role: "ministry" }));
    ...
    it("should fetch and cache federation-specific endpoints", async () => {
      vi.mocked(getUserProfile).mockReturnValue(
        profileFixture({ id: "fed-1", role: "federation" }),
      );

      await seedOfflineCache();

      expect(apiClient.GET).toHaveBeenCalledWith(
        "/api/v1/federation/submissions",
        expect.anything(),
      );
    ...
    it("should fetch and cache apex-specific endpoints", async () => {
      vi.mocked(getUserProfile).mockReturnValue(profileFixture({ id: "apx-1", role: "apex" }));

      // Provide some dummy cooperatives so the loops execute
      vi.mocked(apiClient.GET).mockImplementation(async (url: string) => {
        if (url === "/api/v1/apex/cooperatives") {
          return stubGetResponse({ data: [{ id: "coop-a" }] });
        }
        return stubGetResponse({ data: [] });
      });
      ...
      expect(apiClient.GET).toHaveBeenCalledWith("/api/v1/apex/cooperatives/{id}", {
        params: { path: { id: "coop-a" } },
      });
    });

    it("should fetch and cache cooperative-specific endpoints", async () => {
      vi.mocked(getUserProfile).mockReturnValue(
        profileFixture({ id: "coop-1", role: "cooperative" }),
      );
      ...
      expect(fetchWithAuth).toHaveBeenCalledWith(
        expect.stringContaining("/api/v1/cooperative/profile"),
      );
    });
  });
  ...
  describe("Dynamic Data Looping (Submissions)", () => {
    it("should loop through submissions and fetch their details", async () => {
      ...
      expect(apiClient.GET).toHaveBeenCalledWith("/api/v1/cooperative/submissions/{id}", {
        params: { path: { id: "sub-1" } },
      });
      expect(apiClient.GET).toHaveBeenCalledWith("/api/v1/cooperative/submissions/{id}", {
        params: { path: { id: "sub-2" } },
      });

      // Sections endpoint should be called for BOTH
      expect(apiClient.GET).toHaveBeenCalledWith("/api/v1/cooperative/submissions/{id}/sections", {
        params: { path: { id: "sub-1" } },
      });
      expect(apiClient.GET).toHaveBeenCalledWith("/api/v1/cooperative/submissions/{id}/sections", {
        params: { path: { id: "sub-2" } },
      ...
      expect(apiClient.GET).toHaveBeenCalledWith("/api/v1/cooperative/financial-statements/{id}", {
        params: { path: { id: "fs-1" } },
      });
