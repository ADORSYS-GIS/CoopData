...
const Avatar = ({ name, size = "md" }: { name: string; size?: "sm" | "md" | "lg" }) => {
  ...
  const sz = size === "sm" ? "size-7 text-[10px]" : size === "lg" ? "size-12 text-base" : "size-9 text-xs";
  return (
    <div className={`${sz} rounded-full bg-gradient-to-br ${color} grid place-items-center font-bold text-white shrink-0 ring-2 ring-white`}>
      {initials}
    </div>
  );
};
...
export const ApexUsersPage: React.FC = () => {
  ...
  const handleInvite = () => {
    ...
    addMember.mutate(
      { apexId, email: email.trim(), first_name: firstName.trim(), last_name: lastName.trim(), role: APEX_ROLE },
      {
        onSuccess: () => {
  ...
  const handleSaveEdit = () => {
    ...
    updateMember.mutate(
      { apexId, userId: editingMember.id, first_name: editFirst.trim(), last_name: editLast.trim() },
      {
        onSuccess: () => {
    ...
    >
      ...
      {editingMember && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div onClick={() => setEditingMember(null)} className="absolute inset-0 bg-background/70 backdrop-blur-sm" />
          <div className="relative w-full max-w-sm rounded-2xl border border-border bg-surface p-6 shadow-[var(--shadow-elev-3)] z-10 animate-panel">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{editingMember.email ?? editingMember.id}</p>
              </div>
              <button onClick={() => setEditingMember(null)} className="ml-auto rounded-lg p-1.5 text-muted-foreground hover:bg-muted transition-colors">
                <X className="size-4" />
              </button>
            </div>
            ...
            <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-border">
              <button onClick={() => setEditingMember(null)} className="px-4 py-2 rounded-xl border border-border text-xs font-semibold text-foreground hover:bg-muted/40 transition-colors">Cancel</button>
              <button
      ...
      {confirmRemove && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div onClick={() => setConfirmRemove(null)} className="absolute inset-0 bg-background/70 backdrop-blur-sm" />
          <div className="relative w-full max-w-sm rounded-2xl border border-border bg-surface p-6 shadow-[var(--shadow-elev-3)] z-10 animate-panel">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              Remove <span className="font-semibold text-foreground">{displayName(confirmRemove)}</span> from this apex? They will lose access immediately.
            </p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setConfirmRemove(null)} className="px-4 py-2 rounded-xl border border-border text-xs font-semibold text-foreground hover:bg-muted/40 transition-colors">Cancel</button>
              <button
