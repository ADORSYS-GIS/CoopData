      ...
      <div className="w-full max-w-md bg-surface rounded-2xl border border-border shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-200 overflow-hidden">
        ...
        <div className="px-6 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
          <PeriodSelector
            ...
            onFiscalStartChange={setFiscalStartMonth}
          />
...
function NewApexSubmissionModal({ onClose }: { onClose: () => void }) {
  ...
  const [periodType, setPeriodType] = useState<SubmissionPeriodType>("YEARLY");
  const [periodValue, setPeriodValue] = useState<string>(String(currentYear));
  const [fiscalStartMonth, setFiscalStartMonth] = useState<number>(1);
  const createApexSubmission = useCreateApexSubmission();
  const { data: cooperatives = [], isLoading: coopsLoading } = useCooperatives();
  const { data: apexSubmissions = [] } = useApexSubmissions(true);
  const coopSubmissions = apexSubmissions.filter((s) => s.cooperative_id === selectedCoopId);
  const lock = selectedCoopId ? yearLock(coopSubmissions, year) : null;

  const handlePeriodTypeChange = (type: SubmissionPeriodType) => {
    setPeriodType(type);
    if (type === "YEARLY") setPeriodValue(String(year));
    else if (type === "QUARTERLY") setPeriodValue("Q1");
    else if (type === "MONTHLY") setPeriodValue("01");
    else setPeriodValue("H1");
  };

  const handleYearChange = (y: number) => {
    setYear(y);
    const yearLocked = selectedCoopId ? yearLock(coopSubmissions, y) : null;
    if (yearLocked) {
      handlePeriodTypeChange(yearLocked.periodType);
      if (yearLocked.periodType === "YEARLY") setPeriodValue(String(y));
    } else if (periodType === "YEARLY") {
          ...
          {selectedCoop && (
            <div className="rounded-xl bg-muted/30 border border-border/60 px-4 py-3">
              ...
              <p className="text-xs text-muted-foreground">
                {formatPeriodBadge(periodType, periodValue, year, fiscalStartMonth)}
              </p>
            </div>
          )}
        </div>
...
} from "@/components/submissions/PeriodSelector";
