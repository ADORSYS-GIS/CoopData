...
export const ConsolidatedCoopDetailSheet: React.FC<ConsolidatedCoopDetailSheetProps> = ({ data }) => {
  ...
  const renderKpi = (val: number | undefined, name: string) => {
    ...
    if (name === "par30") colorClass = val <= 5 ? "text-green-600" : val <= 10 ? "text-amber-600" : "text-red-600 font-bold";
    if (name === "car") colorClass = val >= 10 ? "text-green-600" : val >= 8 ? "text-amber-600" : "text-red-600 font-bold";
    if (name === "roa") colorClass = val >= 3 ? "text-green-600" : val >= 1 ? "text-amber-600" : "text-red-600 font-bold";
    if (name === "roe") colorClass = val >= 8 ? "text-green-600" : val >= 4 ? "text-amber-600" : "text-red-600 font-bold";
    if (name === "oer") colorClass = val <= 5 ? "text-green-600" : val <= 8 ? "text-amber-600" : "text-red-600 font-bold";
    
    return (
      <span className={colorClass}>
        {fmtPct(val)}{renderStatus(val, name)}
      </span>
    );
  };
    ...
    <div className="relative flex flex-col w-[210mm] min-h-[296mm] p-12 bg-white break-after-page">
      <div>
        ...
        <table className="w-full text-left text-[9px] mb-8 border-collapse border border-slate-300">
          ...
          <tbody>
            {cooperatives.map((coop: any, i: number) => {
              const kpis = coop.kpis || {};
              const bgClass = i % 2 === 0 ? "bg-white" : "bg-slate-50";
              return (
                <tr key={coop.cooperative_id} className={bgClass}>
                  <td className="p-2 border border-slate-300 font-bold truncate max-w-[120px]">{coop.name}</td>
                  ...
                  <td className="p-2 border border-slate-300 text-right">{fmtNum(kpis.total_assets?.value)}</td>
                  <td className="p-2 border border-slate-300 text-right">{fmtNum(kpis.gross_loan_portfolio?.value)}</td>
                  <td className="p-2 border border-slate-300 text-right">{fmtNum(kpis.total_member_deposits?.value)}</td>
                  <td className="p-2 border border-slate-300 text-right">{fmtNum(kpis.total_equity?.value)}</td>
                  <td className="p-2 border border-slate-300 text-right">{fmtNum(kpis.net_surplus?.value)}</td>
                  <td className="p-2 border border-slate-300 text-right whitespace-nowrap">{renderKpi(kpis.par30?.value, "par30")}</td>
                  <td className="p-2 border border-slate-300 text-right whitespace-nowrap">{renderKpi(kpis.capital_adequacy_ratio?.value, "car")}</td>
                  <td className="p-2 border border-slate-300 text-right whitespace-nowrap">{renderKpi(kpis.roa?.value, "roa")}</td>
                  <td className="p-2 border border-slate-300 text-right whitespace-nowrap">{renderKpi(kpis.roe?.value, "roe")}</td>
                  <td className="p-2 border border-slate-300 text-right whitespace-nowrap">{renderKpi(kpis.operating_expense_ratio?.value, "oer")}</td>
                </tr>
              );
            })}
          </tbody>
