...
// Fully self-contained password card — no hook, no mutation, plain fetch
  ...
  const handle = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/v1/me/password`, {
        ...
        body: JSON.stringify({ current_password: current, new_password: next, logout_sessions: false }),
      });
      const json = await res.json().catch(() => ({})) as { message?: string; error?: string };
      if (!res.ok) { toast.error(json.message ?? json.error ?? `Error ${res.status}`); return; }
...
export const ProfilePage: React.FC = () => {
  ...
  const currentRole = ROLES.find((r) => r.id === role) || { label: "Workspace Member", description: "Standard account" };

  const getRoleCapabilities = () => {
    switch (role) {
      case "ministry": return [
        { name: "User Account Provisioning", allowed: true, scope: "Regional & Cooperative managers" },
        { name: "Filing Approvals / Sign-off", allowed: true, scope: "National compliance scale" },
      ...
      case "federation": return [
        { name: "User Account Provisioning", allowed: true, scope: "Cooperative Managers under federation" },
  ...
  const sessionLogs = [
    { date: "Today, 10:02 AM", ip: "197.104.22.84", device: "Chrome 124.0 (Linux x86_64)", deviceType: "desktop" as const, region: user.region, status: "active" as const },
    { date: "Yesterday, 02:40 PM", ip: "197.104.22.84", device: "Chrome 124.0 (Linux x86_64)", deviceType: "desktop" as const, region: user.region, status: "closed" as const },
    { date: "June 14, 2026, 09:12 AM", ip: "196.44.110.12", device: "Firefox 120 (Android)", deviceType: "mobile" as const, region: "Manzini", status: "closed" as const },
    { date: "June 10, 2026, 08:30 AM", ip: "197.104.22.84", device: "Safari 17 (macOS)", deviceType: "desktop" as const, region: user.region, status: "closed" as const },
  ];
  ...
  return (
    <AppShell title="User Profile" subtitle="Manage your identity settings and security configurations">
      <div className="space-y-6 max-w-5xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 mt-3 text-xs text-muted-foreground">
                <span className="inline-flex items-center gap-1.5"><Mail className="size-3.5 text-accent" /> {user.email}</span>
                <span className="inline-flex items-center gap-1.5"><Locate className="size-3.5 text-accent" /> {user.region} Region</span>
                <span className="inline-flex items-center gap-1.5"><Shield className="size-3.5 text-accent" /> {allowedCount} of {capabilities.length} permissions granted</span>
              </div>
            </div>
          </div>
        </Card>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            <Card title="Organization Details" subtitle="Workspace association" edge="info">
              <div className="space-y-0">
                {renderOrgDetails().map((row, i) => (
                  <div key={row.label} className={`flex justify-between items-start gap-3 py-3 ${i > 0 ? "border-t border-border" : ""}`}>
                    <span className="text-xs text-muted-foreground shrink-0">{row.label}</span>
                    <span className={`text-xs text-right ${row.mono ? "font-mono text-[11px]" : "font-semibold"} text-foreground`}>{row.value}</span>
                  </div>
                ))}
              </div>
            </Card>

            <Card title="Security Preferences" subtitle="Account protection settings" edge="warning">
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3.5 rounded-xl border border-border bg-muted/30">
                  <div className="space-y-1 pr-4">
                    ...
                    <p className="text-xs text-muted-foreground">Require secondary email OTP verification on login</p>
                  </div>
                  <button
                    onClick={() => { setMfaActive(!mfaActive); toast.success(`MFA ${!mfaActive ? "ENABLED" : "DISABLED"}`); }}
                  ...
                  >
                    <span className={`pointer-events-none inline-block size-[18px] rounded-full bg-surface shadow-sm transition-transform ${mfaActive ? "translate-x-[18px]" : "translate-x-0"}`} />
                  </button>
                </div>
                <div className="p-3.5 rounded-xl border border-border bg-muted/30 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-foreground">Auto-Lockout Session</span>
                    </div>
                    <span className="text-sm font-mono font-bold text-accent tabular-nums">{sessionTimeout} min</span>
                  </div>
                  <input type="range" min="15" max="180" step="15" value={sessionTimeout} onChange={(e) => setSessionTimeout(parseInt(e.target.value))} className="w-full accent-accent h-1.5 rounded-lg outline-none cursor-pointer" />
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card title="Role Scope & Ecosystem Permissions" subtitle="Your explicit security credentials matrix" edge="accent">
                      ...
                      <tr key={i} className="hover:bg-muted/30 transition-colors">
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2.5">
                            <div className={`size-1.5 rounded-full shrink-0 ${cap.allowed ? "bg-success" : "bg-destructive"}`} />
                            <span className="font-semibold text-foreground">{cap.name}</span>
                          </div>
                        </td>
                        <td className="px-5 py-3 text-center">
                          <StatusPill tone={cap.allowed ? "success" : "danger"}>{cap.allowed ? "Allowed" : "Restricted"}</StatusPill>
                        </td>
        ...
        {/* Session Logs */}
        <Card title="Recent Session Access Logs" subtitle="Verify locations and devices utilized for account login" edge="none">
          <div className="-mx-5 -mb-5 overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-y border-border bg-muted/60 text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">
                  ...
                  <tr key={i} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2 text-muted-foreground">
                        {log.deviceType === "desktop" ? <Monitor className="size-3.5 text-accent shrink-0" /> : <Smartphone className="size-3.5 text-accent shrink-0" />}
