...
| Pull a model | `docker compose exec ollama ollama pull qwen2.5vl:3b` |
...
```bash
sudo ./scripts/setup-ollama-gpu.sh qwen2.5vl:32b
```

This installs the NVIDIA driver (if needed), installs Ollama, binds it to
`0.0.0.0:11434`, and pulls the model.

### 2.3 Production uses the standalone vLLM module (not Ollama)

Production inference runs in the **`ai/` module** (vLLM), deployed separately
on the GPU instance. `start-prod.sh` does **not** run Ollama — it only verifies
the vLLM endpoint is reachable.

1. Deploy the `ai/` module on the GPU instance (see `ai/README.md`).
2. Set the AI vars in `.env` to point at the vLLM endpoint:

```dotenv
EXTRACTION_BACKEND=llm
AI_PROVIDER_URL=http://<gpu-instance-ip>:8000/v1
...
3. Run `./start-prod.sh` — it starts the app stack and checks the vLLM
   endpoint. If the AI module is down, the app still starts (extraction jobs
   fail cleanly until it's back).
