...
- where are thse kpi actually plced in our code base , do we define the formula our self or library for that ?
-  on whih dashboards do we we see these computed api results 
-  depnedind on data moving accross different levels approval and rejections , what becomes of the computed dt  ?
- i  want detail understanding of the codes mapping to the extracted data from the balance sheet





- compare frontend kpi and excel sheet , getting all 
- it should not be done on get 
- computational optimisation 



so now let me take the opprtunity for us to figure out and work here  concerning the  graph of the "Savings, Loans & Assets" and  "Portfolio Overview,  how and where is the data gotten for this graph and also i want to know if it is from financial or non financial data ?  also i want to know the tables where the extracted from the uploaded file stores the 
date and time data o the db 
...
i want us not to go to the apex analytics and make sure we have the corrent info display and necessary so can strip off any info not needed and also we make it professional show what he needs as an apex for al his cooperatives below him 




lerts seed the data base with more differnt user levels with more data . so lets create  3 federations each fed having 2 apexes each fed and under each apexes we have 4 coop each ,  with each coop havin sumssions approved of 2026,2025 and 2024   so let all the data be consistent and well linked  so we could see the anlyses very well 
