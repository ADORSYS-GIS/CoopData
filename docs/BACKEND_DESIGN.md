...
import { pgTable, uuid, varchar, text, boolean, numeric, date, timestamp, integer, jsonb, pgEnum } from "drizzle-orm/pg-core";
...
export const fdStatusEnum = pgEnum("fd_status", [
  "Active",
  "Matured",
  "Withdrawn",
  "Rolled Over",
...
export const withdrawalFrequencyEnum = pgEnum("withdrawal_frequency", [
  "Low",
  "Medium",
  "High",
]);
...
export const accountingYearEnum = pgEnum("accounting_year", [
  "calendar",
  "fiscal",
]);
...
export const balanceSheetLineItems = pgTable("balance_sheet_line_items", {
  id: uuid("id").primaryKey().defaultRandom(),
  financialStatementId: uuid("financial_statement_id").notNull().references(() => financialStatements.id),
...
export const fixedDeposits = pgTable("fixed_deposits", {
  ...
  singleDepositorDependencyFlag: boolean("single_depositor_dependency_flag").notNull().default(false),
  interestRate: numeric("interest_rate", { precision: 5, scale: 2 }).notNull(),
  balance: numeric("balance", { precision: 15, scale: 2 }).notNull(),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
